...
class Matching_Loss(nn.Module):
    ...
    def forward(self, input, target, mask0, mask1):
        ...
        input_all[:, :, :self.max_object, :self.max_object] = torch.max(
            input_pre, input_next,
        ...
        if int(target_num_pre.data.item()):
            loss_pre = - (
                target_pre * torch.log(input_pre)
            ).sum() / target_num_pre
        else:
            loss_pre = - (target_pre * torch.log(input_pre)).sum()
        if int(target_num_next.data.item()):
            loss_next = - (
        ...
        if mask_next_num:
            accuracy_next = (
                indexes_next[mask_next[:, :, :-1]] == indexes_[
                    mask_next[:, :, :-1]
                ]
            ).float().sum() / mask_next_num
        else:
            accuracy_next = (
                indexes_next[mask_next[:, :, :-1]]
                == indexes_[mask_next[:, :, :-1]]
            ).float().sum() + 1

        return loss_pre, loss_next, loss_similarity, \
            (loss_pre + loss_next + loss + loss_similarity) / \
            4.0, accuracy_pre, accuracy_next, (
                accuracy_pre +
                accuracy_next
            )/2.0, indexes_pre
